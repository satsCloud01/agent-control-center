...
# Default skill used when no matching skill is found
...
class SkillParser:
    ...
    def parse_text(self, content: str, source: str = "<inline>") -> SkillDefinition:
        """Parse skill definition from raw text (for UI upload)."""
        match = FRONTMATTER_RE.match(content)
        if not match:
...
class SkillRegistry:
    ...
    def find_best_match(self, required_skills: list[str]) -> SkillDefinition:
        """Find the skill that best matches the required capabilities."""
