...
from agent_control_center.models.agent_models import AgentStatus
from agent_control_center.orchestration.graph_builder import build_agent_graph
from agent_control_center.orchestration.state import SupervisorState
...
def build_supervisor_graph(
    ...
    async def assign_node(state: SupervisorState) -> dict:
        """Create and assign agents to each subtask."""
        active_agents = []
        updated_subtasks = []
    ...
    async def execute_node(state: SupervisorState) -> dict:
        """Execute all assigned agents concurrently."""
        tasks = []
        task_map = {}  # agent_id -> subtask index
